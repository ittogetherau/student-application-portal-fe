...
export enum ApplicationStatus {
  ...
  FURTHER_DOCUMENTS_REQUESTED = 'further_documents_requested',
  REJECTED = 'rejected',
}
...
export enum CommunicationThreadStatus {
  PENDING = 'pending',
  UNDER_REVIEW = 'under_review',
  COMPLETED = 'completed',
}

export enum CommunicationIssueType {
  DOCUMENT_UNCLEAR = 'document_unclear',
  INFORMATION_INCORRECT = 'information_incorrect',
  DOCUMENT_MISSING = 'document_missing',
  INFORMATION_APPEARS_FAKE = 'information_appears_fake',
  OTHER = 'other',
}
...
export interface CommunicationThread {
  id: string;
  applicationId: string;
  issueType: CommunicationIssueType;
  targetSection: string; // e.g., 'personal-information', 'passport', 'academic-documents'
  targetField?: string; // Specific field if applicable
  subject: string;
  status: CommunicationThreadStatus;
  priority?: 'low' | 'medium' | 'high' | 'urgent';
  deadline?: string; // ISO date string
  markedForReview?: boolean; // Agent marked this thread for staff review
  markedForReviewAt?: string; // ISO date string when marked for review
  createdBy: string;
