...
// schema
export const gsScreeningSchema = z
  .object({
    ...
    intendToApplyStudentVisa: requiredYesNo(
      "Intent to apply for a student visa",
    ),
    ...
    if (values.visaRefusedOrCancelled === "yes") {
      ...
      if (trim(values.visaRefusalDocument) === "") {
        ctx.addIssue({
          ...
          message:
            "Visa refusal or cancellation document reference is required",
        });
      }
    }
...
export type GSScreeningFormValues = z.infer<typeof gsScreeningSchema>;

export type DeclarationViewType = "student" | "agent";

export const createGSScreeningSchema = (viewType: DeclarationViewType) => {
  return gsScreeningSchema.superRefine((values, ctx) => {
    if (viewType === "agent") {
