...
export function AlertsPanel({ staffData }: AlertsPanelProps) {
  ...
  const aggregatedStaffData = useMemo(() => {
    const map = new Map<string, StaffPerformanceItem>();
    for (const item of staffData) {
      const existing = map.get(item.staff);
      if (existing) {
        existing.underReview += item.underReview;
        existing.pendingDecision += item.pendingDecision;
        existing.approved += item.approved;
        existing.rejected += item.rejected;
      } else {
        map.set(item.staff, { ...item });
      }
    }
    return Array.from(map.values());
  }, [staffData]);

  const filteredData = aggregatedStaffData.filter(
          ...
          <div className="flex flex-wrap items-center gap-3 bg-neutral-100 dark:bg-neutral-800 p-1.5 rounded-xl self-start sm:self-auto ring-1 ring-neutral-200 dark:ring-neutral-700">
            <div className="flex items-center gap-1 pr-2 border-r border-neutral-300 dark:border-neutral-700">
              ...
              >
                ...
                {aggregatedStaffData.map((s, idx) => (
                  <option key={`${s.staff}-${idx}`} value={s.staff}>
                    {s.staff}
                  </option>
                ))}
              </select>
