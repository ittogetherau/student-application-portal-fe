...
import { FIELD_MAP } from "../utils/advanced-standing-fields";
...
export default function AdvancedStandingPdfEditor({
  ...
  const renderFields = useCallback(
    (pageData: PdfPageData) => {
      return pageData.annotations
        .filter((annotation) => annotation.fieldName && FIELD_MAP[annotation.fieldName])
        .map((annotation) => {
          const fieldName = annotation.fieldName as string;
          const fieldPath = FIELD_MAP[fieldName];
  ...
  const showBusyOverlay = isDocLoading || (pdfDocEpoch > 0 && pages.length === 0);
      ...
      <div className="space-y-6 p-4">
        {pages.map((page) => (
          ...
          >
            ...
            <div className="absolute inset-0">{renderFields(page)}</div>
          </div>
        ))}
      </div>
    </div>
