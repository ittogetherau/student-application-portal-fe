...
import { Plus, Trash, FileText, Loader2, PenTool, RefreshCw, Eye, X } from "lucide-react";
...
export default function AdvancedStandingForm({
  ...
  const { data: appData, isLoading: isLoadingApp } = useApplicationGetQuery(applicationId);
  ...
  // useWatch reliably subscribes to nested field-array updates (courseEquivalences / basisForCredit).
  const watchedValues =
    (useWatch({ control }) as AdvancedStandingFormValues | undefined) ?? methods.getValues();

  const handleGeneratePreview = useCallback(async () => {
    if (previewGenerationRef.current) return;
    try {
      previewGenerationRef.current = true;
      ...
      previewGenerationRef.current = false;
    }
  ...
  useEffect(() => {
    if (appData?.data && !hasInitializedRef.current) {
      const personal = appData.data.personal_details;
      const enrollment = appData.data.enrollment_data as
  ...
  useEffect(() => {
    if (!previewDialogOpen || pdfBytes) return;
    handleGeneratePreview();
  }, [handleGeneratePreview, pdfBytes, previewDialogOpen]);
  ...
  const onSubmit = async (values: AdvancedStandingFormValues) => {
    try {
      const loadingToast = toast.loading(
        isStaffMode ? "Finalizing assessment..." : "Submitting application..."
      );

      // Generate the PDF file (fields stay live — the uploaded PDF remains editable)
      const pdfFile = await generateAdvancedStandingPdf(values, applicationId, {
        flatten: false,
      });
      ...
      if (!advancedStandingType) {
        toast.error("Document type 'ADVANCE_STANDING' not found. Please contact support.", {
          id: loadingToast,
        });
        return;
      }
      ...
      uploadMutation.mutate(
        {
          ...
          file: pdfFile,
          upload_mode: "replace",
          document_name: isStaffMode
            ? "Advanced Standing Form (Assessed)"
          ...
          onSuccess: () => {
            ...
            updateApplication.mutate(
              ...
              },
              {
                onSuccess: () => {
                  toast.success(
                    isStaffMode ? "Assessment finalized!" : "Form submitted successfully!",
                    { id: loadingToast }
                  );
                  onSuccess?.();
                },
  ...
  /**
   * PDF overlay must update field arrays by replacing the whole array; dotted setValue paths
   * do not reliably sync back to Controllers / useFieldArray in the manual form.
   */
  const handlePdfFieldChange = useCallback(
    (path: string, value: string) => {
      const equivMatch = /^courseEquivalences\.(\d+)\.(.+)$/.exec(path);
      if (equivMatch) {
        const idx = Number(equivMatch[1]);
        const fieldKey = equivMatch[2] as keyof AdvancedStandingFormValues["courseEquivalences"][number];
        const rows = [...(getValues("courseEquivalences") ?? [])];
        while (rows.length <= idx) {
          rows.push({ unitCodeAndName: "", ciheEquivalent: "", approved: "" });
        }
        rows[idx] = { ...rows[idx], [fieldKey]: value };
        setValue("courseEquivalences", rows, {
          shouldDirty: true,
          shouldValidate: true,
          shouldTouch: true,
        });
      ...
      const basisMatch = /^basisForCredit\.(\d+)\.(.+)$/.exec(path);
      if (basisMatch) {
        const idx = Number(basisMatch[1]);
        const fieldKey = basisMatch[2] as keyof AdvancedStandingFormValues["basisForCredit"][number];
        const rows = [...(getValues("basisForCredit") ?? [])];
        while (rows.length <= idx) {
          rows.push({ institution: "", country: "", courseCode: "", courseName: "" });
        }
        rows[idx] = { ...rows[idx], [fieldKey]: value };
        setValue("basisForCredit", rows, {
          shouldDirty: true,
          shouldValidate: true,
          shouldTouch: true,
        });
      ...
      setValue(path as FieldPath<AdvancedStandingFormValues>, value, {
        shouldDirty: true,
        shouldValidate: true,
        shouldTouch: true,
      });
  ...
  return (
    <FormProvider {...methods}>
      <div className="flex flex-col min-h-0 max-h-full bg-background">
        {/* Header */}
        <div className="border-b bg-card shrink-0">
          <div className="w-full max-w-none mx-auto px-0 py-6 flex justify-between items-center">
        ...
        <div className="overflow-y-auto bg-muted/5 min-h-0">
          <div
            className={cn(
              "w-full max-w-none mx-auto px-0 py-6 sm:py-8 space-y-8",
              isStaffMode ? "pb-6" : "pb-12 sm:pb-16"
            )}
          >
            <form
              id="advanced-standing-form"
            ...
            >
              ...
              <Card className="border-primary/10 shadow-md">
                ...
                <CardContent className="p-6 pt-6 space-y-6">
                  <FormRadio
                    ...
                    options={[
                      { label: "Future Student", value: "Future Student" },
                      {
                        ...
                        value: "Currently Enrolled Student",
                      },
                    ]}
                    disabled={true}
                  />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <FormInput
                      name="courseName"
                      label="Churchill Course Name"
                      placeholder="Target Course"
                      disabled={true}
                    />
                    <FormInput name="dateOfBirth" label="Date of Birth" type="date" disabled={true} />
                    <FormInput name="mobile" label="Mobile Phone" placeholder="+61..." disabled={true} />
              ...
              <Card className="border-primary/10 shadow-md">
                ...
                <CardContent className="p-6 pt-6 space-y-4">
                  {basisFields.map((field, index) => (
                    <div
                      key={field.id}
                      className="p-6 rounded-xl border bg-card/50 relative group hover:border-primary/50 transition-colors"
                    >
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <FormInput
                          name={`basisForCredit.${index}.institution`}
                          label="Institution Name"
                          placeholder="University / College"
                        />
                        <FormInput
                          name={`basisForCredit.${index}.country`}
                          label="Country of Study"
                          placeholder="Australia, etc."
                        />
                        <FormInput
                          name={`basisForCredit.${index}.courseCode`}
                          label="Previous Course Code"
                          placeholder="e.g. BSB50420"
                        />
                        <FormInput
                          name={`basisForCredit.${index}.courseName`}
                          label="Previous Course Name"
                          placeholder="e.g. Diploma of Leadership"
                        />
                      </div>
                      {index > 0 && (
                        <Button
              ...
              <Card className="border-primary/10 shadow-md">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    ...
                    {equivalenceFields.length < MAX_COURSE_EQUIVALENCES && (
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          appendEquivalence({ unitCodeAndName: "", ciheEquivalent: "", approved: "" })
                        }
                        className="rounded-full bg-background"
                      >
                ...
                <CardContent className="p-0">
                  <div className="grid grid-cols-1 md:grid-cols-12 bg-muted/30 p-3 border-b text-[10px] font-bold uppercase tracking-wider text-muted-foreground px-4 md:px-6">
                    <div className="md:col-span-4 px-2">Previous Unit Details</div>
                    <div className="md:col-span-5 px-2">Churchill Equivalent Unit</div>
                    <div className="md:col-span-3 px-2 md:text-center">Approved (Y/N)</div>
                  </div>
                  <div className="divide-y px-4 md:px-6">
                    {equivalenceFields.map((field, index) => (
                      <div
                        key={field.id}
                        className="grid grid-cols-1 md:grid-cols-12 gap-4 py-4 items-start group"
                      >
                        <div className="md:col-span-4">
                          <FormInput
                            name={`courseEquivalences.${index}.unitCodeAndName`}
                            label=""
                        ...
                        <div className="md:col-span-5">
                          <FormInput
                            name={`courseEquivalences.${index}.ciheEquivalent`}
                            label=""
                            placeholder="CIHE Unit Title"
                          />
                        </div>
                        <div className="md:col-span-3 flex items-center md:justify-center gap-2 pt-2">
                          {isStaffMode ? (
                            <FormRadio
                ...
                <Card className="border-amber-500/20 shadow-md bg-amber-50/30">
                  <CardHeader className="bg-amber-500/10 border-b border-amber-500/20">
                    <CardTitle className="text-lg flex items-center gap-3 text-amber-900">
                      <span className="flex items-center justify-center w-8 h-8 rounded-full bg-amber-600 text-white font-bold text-sm shadow-sm">
                        5
                      </span>
                  ...
                  <CardContent className="p-6 pt-6 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <FormInput
                        name="staffName"
                        label="Credits Assessed By"
                        placeholder="Staff Full Name"
                      />
                      <FormInput
                        name="staffDate"
                        label="Application Received On / Signature Date"
                        type="date"
                      />
                    </div>
              ...
              <div
                className={cn(
                  "flex flex-wrap items-center justify-center gap-4 pt-8 border-t bg-card/50 p-6 rounded-b-xl",
              ...
              >
                <Button
                  ...
                  className={cn(
                    ...
                    isStaffMode ? "w-full sm:w-64" : "w-full sm:w-auto sm:min-w-[160px]"
                  )}
                >
                  {isPreviewLoading ? (
                ...
                <Button
                  ...
                  className={cn(
                    ...
                    isStaffMode ? "w-full sm:w-64" : "w-full sm:w-auto sm:min-w-[200px]"
                  )}
                >
        ...
        <Dialog open={previewDialogOpen} onOpenChange={setPreviewDialogOpen}>
          <DialogContent className="w-[95vw] max-w-5xl h-[90vh] p-0 overflow-auto flex flex-col border-none">
            <DialogHeader className="p-4 border-b bg-card shrink-0">
              ...
              <VisuallyHidden>
                <DialogDescription>
                  Preview the advanced standing form with editable PDF fields synced to the form
                  data.
                </DialogDescription>
              </VisuallyHidden>
            ...
            <div className="flex-1 bg-muted/20 relative overflow-hidden">
              {pdfBytes ? (
                ...
                <div className="w-full h-full flex flex-col items-center justify-center text-muted-foreground p-12 text-center">
                  ...
                  <p className="text-xs">
                    Your document is being prepared with the latest information...
                  </p>
                </div>
              )}
            </div>
