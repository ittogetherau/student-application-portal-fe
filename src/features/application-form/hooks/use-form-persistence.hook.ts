...
export const useFormPersistence = <T extends FieldValues>({
  ...
  const getMergedStepData = useApplicationFormDataStore(
  ...
  const _hasHydrated = useApplicationFormDataStore(
    ...
    const loadData = () => {
      ...
      if (mergedData) {
        try {
          ...
          console.error(
            `[FormPersistence] Failed to load persisted data for step ${stepId}:`,
            error,
          );
        }
      } else {
        ...
        const tryLoadData = () => {
          ...
          if (retryData) {
            try {
              ...
              console.error(
                `[FormPersistence] Failed to load persisted data on retry for step ${stepId}:`,
                error,
              );
            }
  ...
  useEffect(() => {
    if (!enabled || !applicationId || !_hasHydrated || isInitialLoadRef.current)
      return;
