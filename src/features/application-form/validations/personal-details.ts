...
export const personalDetailsSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.postal_same_as_residential === "No") {
      ...
      if (
        !data.postal_building_name ||
        data.postal_building_name.trim().length === 0
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
      ...
      if (
        !data.postal_street_number ||
        data.postal_street_number.trim().length === 0
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
      ...
      if (
        !data.postal_street_name ||
        data.postal_street_name.trim().length === 0
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
